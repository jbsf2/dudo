defmodule Dudo.Player do
  ...
  def new_player(name, dice_count \\ 5) do
    %__MODULE__{name: name, dice: random_dice(dice_count)}
  end

  def lose_dice(player) do
    %__MODULE__{name: player.name, dice: random_dice(length(player.dice) - 1)}
  end
  ...
  def shake_dice(player), do: %__MODULE__{name: player.name, dice: random_dice(length(player.dice))}
